...
function DiamondNode({ data }: { data: any }) {
  ...
  return (
    <div
      className="relative bg-background border border-border p-4 flex items-center justify-center transform rotate-45 cursor-pointer group"
    ...
    >
      ...
      {isEditing ? (
        <input
          ...
          className="bg-transparent border-none outline-none text-center w-full transform -rotate-45"
          placeholder="输入文字..."
          autoFocus
          style={{ fontSize: `${fontSize}px` }}
        />
      ) : (
        <span className="text-center w-full transform -rotate-45" style={{ fontSize: `${fontSize}px` }}>
          {label || '双击编辑'}
        </span>
      )}
    </div>
